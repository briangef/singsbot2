...
const options = {
    channels: ['dadjokecinema', 'bellaazinha'],
    connection: {
        cluster: 'aws',
        reconnect: true,
    },
    identity: {
        password: "4lpc8voyxica9t5hq5de8e1eeqytcw",
        username: 'SingsBot',
    },
    options: {
        debug: true,
...
router.get("queue", "/queue/:channel", async (ctx) => {
    const thischannel = ctx.params.channel;
    const channelqueue = QueueList["#" + thischannel];
